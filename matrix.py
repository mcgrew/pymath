...
class matrix( object ):
    ...
    '''
    #constants
    _VALID_TYPES = ( types.IntType, types.FloatType, types.LongType )
    _VALID_TYPENAMES = ( 'int', 'float', 'long')
    _VALID_COLLECTIONS = ( types.ListType, types.TupleType )
    ...
    def __init__( self, *rows ):
        ...
        if rows:
            ...
            elif ( ( len( rows ) == 1 ) and ( type( rows[ 0 ] ) == types.ListType ) and ( type( rows[ 0 ][ 0 ] == types.ListType ) ) ):
                for row in rows[ 0 ]:
                    newRow = list( )
                    for item in row: # this should make a deep copy.
                        newRow.append( item )
            ...
            elif ( ( len( rows ) == 2 ) and ( type( rows[ 0 ] ) == types.ListType ) and ( type( rows[ 1 ] ) == types.IntType ) ):
            ...
            # if the value passed into the constructor is several lists
                ...
                for row in rows:
                    if not ( type( row ) == types.ListType ):
                        raise TypeError( "Constructor arguments must be of type 'list'" ) # fix this!
                    self.addRow( *row )
    ...
    def __div__( self, obj ):
        ...
        '''
        if ( type( obj ) in self._VALID_TYPES ):
            returnvalue = matrix( )
            for row in self._value:
                newRow = list( )
                for item in row:
                    newItem = item / obj
                    # convert all of the even floats to int.
    ...
    def __getitem__( self , index ):
        ...
        '''
        if not ( type( index ) == types.IntType ):
            return NotImplemented
    ...
    def __mod__( self, mod ):
        ...
        '''
        if not ( type( mod ) in  self._VALID_TYPES ):
            return NotImplemented
        returnvalue = matrix( )
        for i in range( self._height ):
            currentRow = list( )
    ...
    def insertColumn( self, index, *column ):
        ...
        :Parameters:
            ...
            column : list
                The column to be inserted
        '''
        if ( ( len( column ) == 1 ) and ( type( column[0] ) == types.ListType ) ):
            column = column[0]
        if self._height:
        ...
        self._width += 1
        for i in range( self._height ):
            if not ( type( column[ i ] ) in self._VALID_TYPES ):
                message = "Values must be of type "
                for t in range( len( self._VALID_TYPENAMES ) ):
                    if t:
                        message += ' or '
                    message += "'%s'" % self._VALID_TYPENAMES[ t ]
    ...
    def insertRow( self, index, *row ):
        ...
        :Parameters:
            ...
            row : list
                The row to be inserted
        '''
        if ( ( len( row ) == 1 ) and ( type( row[0] ) == types.ListType ) ):
            row = row[0]
        if self._width:
        ...
        newrow = list( )
        for item in row:
            if not ( type( item ) in self._VALID_TYPES ):
                message = "Values must be of type "
                for t in range( len( self._VALID_TYPENAMES ) ):
                    if t:
                        message += ' or '
    ...
    def inverse( self ):
        ...
        return ( self.adjoint( ).itemsToFloat( ) / self.determinant( ) )
    ...
    # Returns True or False whether this matrix is Square.
    def isSquare( self ):
        '''
    ...
    def swapColumns( self, i, j ):
        ...
        '''
        if not ( type( i ) == types.IntType and type( j ) == types.IntType ):
